...
- [x] Commit this empty version of the package to the `feat/reference-learning-mvp` branch.
...
- [x] Initialize the controller with the `kHardcodedVerses` data.

- [x] Implement methods: `nextVerse()`, `previousVerse()`, `submitReference(input)`, `toggleReferenceVisibility()`.

- [x] Implement getters for `currentVerse`, `userInputReference`, `isInputCorrect`, `isReferenceVisible`.
...
### Phase 7: BDD `.feature` Files

-   [ ] Create a `features` directory in the package root.
-   [ ] Create `features/learn_reference.feature` describing the core user flow: display verse, type reference, get feedback, navigate.
...
- [ ] Create `examples/simple_demo` (very thin wrapper, supports all platforms).
    ...
    - [ ] Implement a basic `main.dart` that displays the `ReferenceLearningPage`.

- [ ] Create `examples/demo_app` (slightly more feature-rich, supports all platforms).

    - [ ] Use `create_project` to generate the app.

    - [ ] Add `reference_learning` as a local dependency.

    - [ ] Implement a `main.dart` that displays the `ReferenceLearningPage` (ready for future Firebase expansion).

- [ ] Ensure both apps run on iOS, Android, Web, Windows, macOS, and Linux.
...
-   Created feature branch `feat/reference-learning-mvp`.
